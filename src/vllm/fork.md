...
### Consolidation status (Sep 3–23 2026) {#consolidation-status}
...
| **LMCache** | Still **no published gfx1030 recipe**. `#vllm-rdna` / `#general` Sep 18–19: do **not** build the connector inside official `rocm/pytorch` images (they lack HIP/dev tooling). Intended shape: [`lmcache/standalone`](https://hub.docker.com/r/lmcache/standalone) in **CPU** mode ([standalone starter](https://docs.lmcache.ai/getting_started/quickstart/standalone_starter.html)) + LMCache connector on a **bare-metal / venv** `rdna_extras` serve (or a patched `vllm-rdna` image). Official CUDA image `lmcache/vllm-openai` is **not** the RDNA path. `#vllm-rdna` (Sep 22) / `#lmcache` (Sep 23): another V620 compile still failed; a community LDS **~143 KB vs 64 KB** claim is **Needs verify**. Last known real blocker was **Mamba / SSM alignment**. Community: **upstream vLLM CPU/SSD KV offload** (not LMCache) fell to **~1 t/s** decode — [troubleshooting](../../troubleshooting/vllm.md#upstream-kv-offload-tanks-decode). |
...
- `fa_rdna2.cu` — a **FlashAttention** kernel tuned for RDNA2, exposed via
  ...
  and caused Triton GDN compile hangs). `#vllm-rdna` (Sep 22): when the extras fork went public the
  maintainer **dropped the FP16 FA pieces** to focus on validating **W4A16**. Independent public
  kernel [`2kiss/flash-attention-rdna2`](https://github.com/2kiss/flash-attention-rdna2) showed up
  the same day — community impressed, **no gfx1030 bench posted yet**. Org target remains
  **improving in-tree `fa_rdna2`**, not swapping the backend. **Needs verify.**
