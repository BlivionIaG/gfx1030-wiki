...
| **Flash-Next production** | **4× V620** still the vLLM path; prefer **latest `rdna_extras`** (now includes `#17` + `#20`) over a stale leapdragon checkout. Published containers may lag. **3× PP3:** leapdragon image decoded correctly; `rdna_extras` still **garbage** on a Sep 16 fresh pull — [troubleshooting](../troubleshooting/vllm-flash-next.md#flash-next-pp3-output-corruption). Community Sep 17–18 overlay [`alanoo81/flashnext-v620-pp3`](https://github.com/alanoo81/flashnext-v620-pp3) (leapdragon + org MoE HIP) — [recipe](flash-next-serve.md#flash-next-3x-moe-hip-overlay). `#general` (Sep 23): overlay author is **no longer working on it**; multi-agent can hit **infinite KV-pool dump loops** — cap concurrent agents. **Default graphs:** still **`FULL_DECODE_ONLY`** from `#17`. **Merged** [`#20`](https://github.com/opengfx1030/vllm-rdna/pull/20) (24 Sep) makes compiled **PIECEWISE** / `FULL_AND_PIECEWISE` boot and stay correct, but decode dropped **~63–70 → ~26–34 t/s** (community: ~2.5× slower) — keep `#17` graphs until a graph-launch follow-up. [`#21`](https://github.com/opengfx1030/vllm-rdna/pull/21) (FULL decode + piecewise) was **closed without merge**. |
