...
### Consolidation status (Sep 3–20 2026) {#consolidation-status}
...
| **LMCache** | Still **no published gfx1030 recipe**. `#vllm-rdna` / `#general` Sep 18–19: do **not** build the connector inside official `rocm/pytorch` images (they lack HIP/dev tooling). Intended shape: [`lmcache/standalone`](https://hub.docker.com/r/lmcache/standalone) in **CPU** mode ([standalone starter](https://docs.lmcache.ai/getting_started/quickstart/standalone_starter.html)) + LMCache connector on a **bare-metal / venv** `rdna_extras` serve (or a patched `vllm-rdna` image). Official CUDA image `lmcache/vllm-openai` is **not** the RDNA path. Community: **upstream vLLM CPU/SSD KV offload** (not LMCache) fell to **~1 t/s** decode — [troubleshooting](../../troubleshooting/vllm.md#upstream-kv-offload-tanks-decode). |
